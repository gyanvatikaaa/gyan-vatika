...
const Review = () => {
  ...
  const [editingAnswer, setEditingAnswer] = useState(null); // { submissionId, questionIndex }
  const [editMarks, setEditMarks] = useState("");
  const [editRemark, setEditRemark] = useState("");
  const [savingMarks, setSavingMarks] = useState(false);

  const startEditingMarks = (submissionId, answer) => {
    setEditingAnswer({ submissionId, questionIndex: answer.questionIndex });
    setEditMarks(answer.marksAwarded ?? 0);
    setEditRemark(answer.aiRemark || "");
  };
  ...
  const saveMarks = async () => {
    ...
    setError("");
    try {
      await api.patch(`/review/${editingAnswer.submissionId}/answer/${editingAnswer.questionIndex}`, {
        marksAwarded: Number(editMarks),
        remark: editRemark,
      });
                      ...
                      {a.photoUrls?.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-1">
                          {a.photoUrls.map((url, pi) => (
                            <img key={pi} src={url} alt={`Answer ${i + 1} page ${pi + 1}`} className="w-24 h-24 object-cover rounded-lg border border-vatika-line" />
                          ))}
                        </div>
                      )}
                      {a.aiRemark && <p className="text-xs text-vatika-muted italic mb-2">{a.aiRemark}</p>}

                      {editingAnswer?.submissionId === sub._id && editingAnswer?.questionIndex === a.questionIndex ? (
                        <div className="mt-2 space-y-2">
                          <input
                            ...
                            className="w-24 rounded-lg border border-vatika-line px-2 py-1 text-sm"
                          />
                          <textarea
                            value={editRemark}
                            onChange={(e) => setEditRemark(e.target.value)}
                            rows={2}
                            className="w-full rounded-lg border border-vatika-line px-2 py-1 text-sm"
                          />
                          <div className="flex gap-2">
                            <button
                              onClick={saveMarks}
                              disabled={savingMarks}
                              className="text-xs bg-vatika-forest text-white rounded-lg px-3 py-1.5 hover:bg-vatika-forestDark transition disabled:opacity-60"
                            >
                              {savingMarks ? "Saving…" : "Save"}
                            </button>
                            <button
                              onClick={() => setEditingAnswer(null)}
                              className="text-xs border border-vatika-line rounded-lg px-3 py-1.5 text-vatika-muted hover:bg-white transition"
                            >
                              Cancel
                            </button>
                          </div>
                        ...
                        <button
                          onClick={() => startEditingMarks(sub._id, a)}
                          className="text-xs text-vatika-forest hover:underline"
                        >
                          Edit marks
                        </button>
                      )}
