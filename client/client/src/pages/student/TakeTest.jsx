...
const TakeTest = () => {
  ...
  const handlePhotoChange = async (index, file) => {
    ...
    try {
      ...
      updateAnswer(index, "photoUrl", data.url);
    } catch (err) {
          ...
          <div key={i} className="bg-vatika-surface border border-vatika-line rounded-card p-5">
            ...
            {q.type === "mcq" ? (
              ...
              <div>
                ...
                <p className="text-xs text-vatika-muted mb-2">— or upload a photo of your handwritten answer —</p>
                ...
                {answers[i]?.photoUrl && (
                  <div className="mt-3">
                    <img
                      src={answers[i].photoUrl}
                      alt={`Answer ${i + 1}`}
                      className="max-w-xs rounded-lg border border-vatika-line"
                    />
                    <button
                      type="button"
                      onClick={() => updateAnswer(i, "photoUrl", "")}
                      className="block text-xs text-vatika-clay mt-1 hover:underline"
                    >
                      Remove photo
                    </button>
                  </div>
