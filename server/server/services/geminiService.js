...
IMPORTANT formatting rule: Never use LaTeX notation or dollar signs ($) for math or scientific expressions.
Write all math/formulas in plain text using standard keyboard symbols only, e.g. write "x^2 + 3x - 5 = 0",
"sqrt(16)", "22/7", "H2O", "a/b", "pi", "degrees", not "$x^2$" or "\\(x^2\\)" or "\\frac{a}{b}".
...
Respond with ONLY valid JSON in exactly this shape, no extra commentary:
{
  ...
  // Safety net: strip any LaTeX-style syntax Gemini might still slip in,
  // since the UI displays plain text and can't render $...$ or \frac{}{} etc.
  ...
  parsed.questions = parsed.questions.map((q) => ({
    ...q,
    questionText: stripLatex(q.questionText),
    options: Array.isArray(q.options) ? q.options.map(stripLatex) : q.options,
...
// Evaluates one short/long answer — either typed text or one/more photos of handwriting.
// Returns { marksAwarded, remark } where remark explains mistakes / improvements.
async function evaluateAnswer({ questionText, maxMarks, textAnswer, photoUrls }) {
...
{ "marksAwarded": number (0 to ${maxMarks}), "remark": "string, 1-2 sentences explaining the score, mistakes, and how to improve" }`;
  ...
  if (photoUrls && photoUrls.length > 0) {
    const images = await Promise.all(photoUrls.map((url) => fetchImageAsBase64(url)));
    parts = [
      {
        text:
          instructions +
          `\n\nThe student's answer is handwritten, spread across ${images.length} photo(s) attached in order. Read all of them together as one continuous answer, then grade it.`,
      },
      ...images.map(({ base64, mimeType }) => ({ inlineData: { mimeType, data: base64 } })),
    ];
  } else {
